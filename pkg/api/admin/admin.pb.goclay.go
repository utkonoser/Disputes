...
/*
Package admin is a self-registering gRPC and JSON+Swagger service definition.
...
// AdminAPIDesc is a descriptor/registrator for the AdminAPIServer.
type AdminAPIDesc struct {
	svc  AdminAPIServer
	opts httptransport.DescOptions
}

// NewAdminAPIServiceDesc creates new registrator for the AdminAPIServer.
// It implements httptransport.ConfigurableServiceDesc as well.
func NewAdminAPIServiceDesc(svc AdminAPIServer) *AdminAPIDesc {
	return &AdminAPIDesc{
		svc: svc,
	}
}
...
func (d *AdminAPIDesc) RegisterGRPC(s *grpc.Server) {
	RegisterAdminAPIServer(s, d.svc)
}

// Apply applies passed options.
func (d *AdminAPIDesc) Apply(oo ...transport.DescOption) {
	for _, o := range oo {
		o.Apply(&d.opts)
	}
}
...
func (d *AdminAPIDesc) SwaggerDef(options ...swagger.Option) (result []byte) {
	if len(options) > 0 || len(d.opts.SwaggerDefaultOpts) > 0 {
		var err error
		var s = &spec.Swagger{}
		if err = s.UnmarshalJSON(_swaggerDef_admin_proto); err != nil {
		...
		result = _swaggerDef_admin_proto
	}
	return result
}

// RegisterHTTP registers this service's HTTP handlers/bindings.
func (d *AdminAPIDesc) RegisterHTTP(mux transport.Router) {
...

	{
		...
		h = http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			unmFunc := unmarshaler_goclay_AdminAPI_Health_0(r)
			rsp, err := _AdminAPI_Health_Handler(d.svc, r.Context(), unmFunc, d.opts.UnaryInterceptor)
		...
		if isChi {
			chiMux.Method("GET", pattern_goclay_AdminAPI_Health_0, h)
		} else {
			mux.Handle(pattern_goclay_AdminAPI_Health_0, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				if r.Method != "GET" {
...
type AdminAPI_httpClient struct {
	c    *http.Client
	host string
}

// NewAdminAPIHTTPClient creates new HTTP client for AdminAPIServer.
// Pass addr in format "http://host[:port]".
func NewAdminAPIHTTPClient(c *http.Client, addr string) *AdminAPI_httpClient {
...
func (c *AdminAPI_httpClient) Health(ctx context.Context, in *HealthRequest, opts ...grpc.CallOption) (*HealthResponse, error) {
	...
	path := pattern_goclay_AdminAPI_Health_0_builder(in)
...
var (
	...
	unmarshaler_goclay_AdminAPI_Health_0_boundParams = &utilities.DoubleArray{Encoding: map[string]int{}, Base: []int(nil), Check: []int(nil)}
)

// marshalers for AdminAPI
var (
...
var _swaggerDef_admin_proto = []byte(`{
  "swagger": "2.0",
  "info": {
    "title": "admin.proto",
  ...
  "paths": {
    "/health": {
      "get": {
        ...
        "operationId": "AdminAPI_Health",
        "responses": {
          "200": {
            "description": "A successful response.",
            "schema": {
              "$ref": "#/definitions/adminHealthResponse"
            }
          }
        },
        "tags": [
